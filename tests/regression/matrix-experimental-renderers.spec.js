...
test.describe("Regression: experimental renderers (three.js, p5.js)", () => {
  ...
  test("three-rain preset mathcode_alphabet_three (renderer from version)", async ({
    page,
  }) => {
  ...
  test("three-rain explicit renderer=three&version=mathcode", async ({
    page,
  }) => {
    const q = new URLSearchParams({
      ...base,
    ...
    const webgl = await rainSurfaceCanvas(page).evaluate(
      (el) => !!(el.getContext("webgl2") || el.getContext("webgl")),
    );
  ...
  test("p5-rain preset mathcode_p5", async ({ page }) => {
    ...
    await assertMatrixBootsClean(page, q);
    const ctx2d = await rainSurfaceCanvas(page).evaluate(
      (el) => !!el.getContext("2d"),
    );
    expect(
      ctx2d,
      "p5 2D renderer should expose Canvas 2D on the visible surface",
    ).toBe(true);
    await expect(page.locator("select.version-select")).toHaveValue(
      "mathcode_p5",
      { timeout: 15_000 },
